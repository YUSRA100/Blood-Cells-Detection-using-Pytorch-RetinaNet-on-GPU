    ...
    for img_name in os.listdir(image_path):
        ...
        with torch.no_grad():
            ...
            if len(counter)==0:
                ...
                if "RBC" in counter:    
                    cv2.putText(image_orig, "RBC : "+ str(counter["RBC"]), position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                else:
                    cv2.putText(image_orig, "RBC : "+ str(0), position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                if "Platelets" in counter:
                    cv2.putText(image_orig, "Platelets : "+ str(counter["Platelets"]), (10,70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                else:
                    cv2.putText(image_orig, "Platelets : "+ str(0), (10,70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)  
   
            labellist = []
            cv2.imshow('detections', image_orig)
            cv2.imwrite("output_image"+str(count)+".jpg",image_orig)
